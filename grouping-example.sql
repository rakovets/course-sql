...
SELECT COUNT(*) AS 'count students'
FROM student AS s, student_group AS sg
...
FROM student AS s, student_group AS sg
WHERE sg.group_id = s.group_id; -- Wrong results
...
SELECT sg.name, s.student_grant, COUNT(*) AS 'count students in groups with student\'s grant'
FROM student AS s, student_group AS sg
WHERE sg.group_id = s.group_id
...
FROM student
WHERE AVG(student_grant) <= 1200; -- SQL Error
...
FROM student_group AS sg, student AS s
WHERE sg.group_id = s.group_id
GROUP BY name
...
HAVING student_grant = MAX(student_grant); -- wrong result

SELECT lastname, firstname, name, student_grant
FROM student AS s, student_group AS sg
WHERE sg.group_id = s.group_id AND student_grant = 1256; -- wrong result

SELECT lastname, firstname, name, student_grant
FROM student AS s, student_group AS sg
WHERE sg.group_id = s.group_id AND student_grant = (SELECT MAX(student_grant)
													FROM student);
...
SELECT t.fullname, sq.name
FROM trainer AS t, trainer_subject_link AS tsl, (SELECT s.subject_id, s.name
												FROM subject AS s, achievement AS a
...
SELECT subject.name,(SELECT MAX(a.assessment)
